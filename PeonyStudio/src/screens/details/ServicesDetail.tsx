...
import { addBookServiceAppointmentAPI } from '../../redux/actions/BookServiceAppointmentAction'
...
const ServicesDetail: React.FC = () => {
    ...
    const { services, userID } = route.params
    ...
    const handleConfirm = async () => {
        ...
        const check = await axios.post("https://mobile-react-native-workshop-server.onrender.com/find-book-services-appointment-by-id-user-and-id-services", {
            id_user: userID,
        ...
        if (check.status === 200) {
            if (check.data.status === 3) {
                ...
                dispatch(addBookServiceAppointmentAPI(book))
                setNotifyText('Đặt lịch dịch vụ thành công')
            } else {
                setNotifyText('Dịch vụ đang được lên lịch')
            }
        } else {
            const book = {
                id: GenerateRandomId('BS'),
            ...
            <ImageBackground source={{ uri: uri }} style={{ width, height: height * 0.58 }}>
                <View style={{ margin: 5, flexDirection: 'row', alignContent: 'center' }}>
                    <TouchableOpacity onPress={() => navigation.goBack()}>
                        <AntDesign name='leftcircle' color={Color.pink300} size={35} />
                    </TouchableOpacity>
                </View>
            </ImageBackground>
            <View style={{ margin: 5, flex: 1 }}>
                ...
                >
                    ...
                    <Text style={{ fontSize: 23, color: Color.deepPink, fontWeight: 'bold', paddingBottom: 2, borderBottomWidth: 1, borderStyle: 'dashed', borderColor: Color.pink300 }}>
                        {services.serviceName}</Text>

                    <Text style={{ fontSize: 17, color: Color.purpleA100 }}>
                        Giá dịch vụ: {services.servicePrice} VNĐ
                    </Text>
                    ...
                    {services.describes.map((item, index) => (
                        <View key={index}
